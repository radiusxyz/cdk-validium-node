...
func (f *finalizer) storeL2Block(ctx context.Context, l2Block *L2Block) error {
	...
	log.Infof("Complete building block %d [%d], batch: %d, deltaTimestamp: %d, timestamp: %d, l1InfoTreeIndex: %d, l1InfoTreeIndexChanged: %v, txs: %d/%d, blockHash: %s, infoRoot: %s, time: %v",
		blockResponse.BlockNumber, l2Block.trackingNum, f.wipBatch.batchNumber, l2Block.deltaTimestamp, l2Block.timestamp, l2Block.l1InfoTreeExitRoot.L1InfoTreeIndex,
...
func (f *finalizer) finalizeWIPL2Block(ctx context.Context) error {
	...
	if !f.cfg.UseExternalSequencer {
		...
		fmt.Println("stompesi - blockHeight, sequencerUrlList", blockHeight, sequencerUrlList)
		...
		fmt.Println("stompesi - getRawTxListResponse", getRawTxListResponse)
...
func (f *finalizer) GetSequencerUrlList() (uint64, []string, error) {
	...
	// HTTP POST 요청 전송
	resp, err := http.Post(f.cfg.SeedNodeURI, "application/json", bytes.NewBuffer(reqBytes))
	if err != nil {
...
func (f *finalizer) getRawTxList() (*GetRawTxListResponse, error) {
	...
	fmt.Println("stompesi - rollup_block_height", rollup_block_height)
	fmt.Println("stompesi - sequencerIndex", sequencerIndex)
	fmt.Println("stompesi - sequencerUrlList", sequencerUrlList)
	...
	fmt.Println("stompesi - messageBytes", messageBytes)
	...
	fmt.Println("stompesi - finalize_block", request)
	...
	// HTTP POST 요청 전송
	resp, err := http.Post(sequencerRpcUrl, "application/json", bytes.NewBuffer(reqBytes))
	if err != nil {
	...
	fmt.Println("stompesi - body", body)
	...
	fmt.Println("stompesi - finalize_block done")
	...
	fmt.Println("stompesi - get_raw_transaction_list", request)
	...
	fmt.Println("stompesi - getRawTxListResponse")
